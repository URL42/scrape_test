...
A per-company brief generated by Claude (`claude-opus-5`) that reads everything above and
returns: a news summary written for a seller, an interpretation of the combined signals, a
...
It is **on demand, never automatic**. Each generation costs roughly 4-5 cents, and most
searches are a glance - firing it on every lookup would bill for companies nobody is
working. Briefs are cached against a hash of their own inputs, so re-opening one is free
and only **Regenerate** spends again. Change the model with `SCRAPE_TEST_MODEL`.
...
```bash
uv run pytest          # 63 tests (unit, async cache/retry, brief with a mocked client)
...
- The brief's tests inject a fake model client. They cover prompt assembly, parsing,
  caching and the failure paths, but they do not exercise a live Anthropic call.
